...
resource "local_file" "lab_info" {
    ...
    content     = <<-EOT
    FTD SSH  = ssh -o "StrictHostKeyChecking accept-new" -i "${local_file.this.filename}" admin@${aws_eip.ftd-mgmt-EIP.public_dns}
    APP SSH  = ssh -o "StrictHostKeyChecking accept-new" -i "${local_file.this.filename}" ec2-user@${aws_instance.app.public_dns}
    EOT
