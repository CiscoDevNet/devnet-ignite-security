...
import time  # Added for time.sleep in del_interfaces
...
class AwsPod:
    ...
    def run_all(self):
        ...
        try:
            ...
            print(e)
            # Do not exit here, just increment fail_count for the pod
            self.fail_count += 1
            return self.fail_count
                ...
                try:
                    ...
                    if response.get('Unsuccessful'):
                        for item in response['Unsuccessful']:
                            print(
                                f"  Error deleting EIP {item.get('ResourceId')}: {item.get('Error', {}).get('Message')}")
                            self.fail_count += 1
                except Exception as e:
                ...
                for route in rt['Routes']:
                    if route.get('GatewayId', "").startswith('vpce-'):
                        ...
                        if destination_cidr_block:
                            ...
                            return  # Original code returns after first deletion
                        else:
                            print("Could not determine DestinationCidrBlock for the route to delete.")
                            self.fail_count += 1  # Consider this a failure
                            return
        except Exception as e:
                    ...
                    for endpoint in vpc_endpoint_ids:
                        ...
                        if unsuccessful:
                            ...
                            for entry in unsuccessful:
                                print(
                                    f"  Endpoint ID: {entry.get('ResourceId')}, {entry.get('Error', {}).get('Message')}")
                            self.fail_count += 1
                        else:
            ...
            for instance in reservation['Instances']:
                if self.delete:
                    try:
                        ...
                        print(f"Error terminating instance {instance['InstanceId']}: {e}")
                        self.fail_count += 1
        ...
        for interface in interfaces['NetworkInterfaces']:
            ...
            if 'Association' in interface and 'AllocationId' in interface['Association']:
                ...
                if self.delete:
                    try:
                        ...
                        print(f"Error disassociating/releasing EIP for ENI {eni_id}: {e}")
                        self.fail_count += 1
            ...
            try:
                ...
                if "InvalidNetworkInterfaceID.NotFound" in str(e):
                    ...
                    print(
                        f"Network Interface {eni_id} is still in use and cannot be deleted yet. It might be associated with a terminating instance.")
                    self.fail_count += 1
                else:
            ...
            if self.tag_match in key_pair['KeyName']:
                ...
                if self.delete:
                    try:
                        ...
                        print(f"Error deleting key pair {key_pair['KeyName']}: {e}")
                        self.fail_count += 1
            ...
            for page_version in pages_versions:
                ...
                if 'Versions' in page_version:
                    for obj_version in page_version['Versions']:
                        objects_to_delete_versions.append(
                ...
                if self.delete:
                    if stack['StackStatus'] == 'DELETE_IN_PROGRESS':
                        ...
                        try:
                            ...
                            print(f"Error initiating deletion for stack {stack['StackName']}: {e}")
                            self.fail_count += 1
            else:
                continue
...
class SccPod:
    ...
    def __init__(self, tag_name, pod_number, delete=False):
        if self.api_token is None:
            ...
            # Do not exit here, allow other parts of the script to run if possible
            # This will cause subsequent SCC operations to fail, which will be tracked by fail_count
        self.tag_match = f'{tag_name}-{pod_number}'
        self.autoscale_tag_match = f"{DEFAULT_AUTOSCALE_TAG}-{pod_number}"
        self.delete = delete
        self.fail_count = 0  # Initialize fail_count for each instance

    @classmethod
    def get_pods(cls, tag_match=DEFAULT_TAG):
        if cls.api_token is None:
            print("SCC init Error: please provide api_token for SCC in env variable FMC_TOKEN")
            return []  # Return empty list if API token is missing
                ...
                if tag_match in item['name']:
                    ...
                    try:
                        ...
                        if len(parts) > 2:
                            ...
                            print(
                                f"Warning: Device name '{item['name']}' does not match expected format 'TAG-IGNITE-PODNUMBER'. Skipping.")
                    except IndexError:
                        print(f"Error parsing pod number from device name: {item['name']}")
        return pods
    ...
    def rest_get(cls, url):
        try:
            ...
            print(f"Error in GET {url} --> {err}")
            if r: print(f"Response: {r.text}")
            return None
        except requests.exceptions.RequestException as err:
            print(f"Error in connection for GET {url} --> {err}")
            return None
    ...
    def rest_post(cls, url, post_data):
        try:
            ...
            print(f"Error in POST {url} --> {err}")
            if r: print(f"Response: {r.text}")
            return None
        except requests.exceptions.RequestException as err:
            print(f"Error in connection for POST {url} --> {err}")
            return None
    ...
    def rest_delete(cls, url):
        try:
            ...
            print(f"Error in DELETE {url} --> {err}")
            if r: print(f"Response: {r.text}")
            return None
        except requests.exceptions.RequestException as err:
            print(f"Error in connection for DELETE {url} --> {err}")
            return None

    def _delete_resource(self, api_path_base, resource_type):
        url = f"{self.server}{api_path_base}"
        r = self.rest_get(url)
        if r and 'items' in r.json():
            for item in r.json()['items']:
                if self.tag_match in item['name']:
                    print(f"Found {resource_type}: {item['name']}")
                    if self.delete:
                        delete_url = f"{self.server}{api_path_base}/{item['id']}"
                        delete_r = self.rest_delete(delete_url)
                        if delete_r:
                            print(f"Deleted {resource_type} {item['name']}")
                        else:
                            print(f"Failed to delete {resource_type} {item['name']}")
                            self.fail_count += 1
        elif r is None:
            self.fail_count += 1

    def del_ac_policy(self):
        self._delete_resource("/api/fmc_config/v1/domain/e276abec-e0f2-11e3-8169-6d9ed49b625f/policy/accesspolicies",
                              "AC policy")

    def del_platform_policy(self):
        self._delete_resource(
            "/api/fmc_config/v1/domain/e276abec-e0f2-11e3-8169-6d9ed49b625f/policy/ftdplatformsettingspolicies",
            "Platform policy")

    def del_intrusion_policy(self):
        self._delete_resource("/api/fmc_config/v1/domain/e276abec-e0f2-11e3-8169-6d9ed49b625f/policy/intrusionpolicies",
                              "Intrusion policy")

    def del_objects(self):
        self._delete_resource("/api/fmc_config/v1/domain/e276abec-e0f2-11e3-8169-6d9ed49b625f/object/hosts",
                              "Host Object")
        self._delete_resource("/api/fmc_config/v1/domain/e276abec-e0f2-11e3-8169-6d9ed49b625f/object/networks",
                              "Network Object")
        self._delete_resource("/api/fmc_config/v1/domain/e276abec-e0f2-11e3-8169-6d9ed49b625f/object/securityzones",
                              "Security Zone")
        ...
        if r and 'items' in r.json():
            for item in r.json()['items']:
                if self.tag_match in item['name'] or self.autoscale_tag_match in item['name']:
                    ...
                    if self.delete:
                        delete_api_path = f"/firewall/v1/inventory/devices/ftds/cdfmcManaged/{item['uid']}/delete"
                        delete_url = f"{self.scc_server}{delete_api_path}"
                        delete_r = self.rest_post(delete_url, {})  # POST with empty body
                        if delete_r:
                            print(f"Deleted device {item['name']}")
                        else:
                            print(f"Failed to delete device {item['name']}")
                            self.fail_count += 1
        elif r is None:
            self.fail_count += 1
...
def cleanup_pod(pod_number, delete):
    ...
    for i in range(1, ITERATION_COUNT + 1):
        ...
        overall_fail_count += scc.run_all()

        aws_pod = AwsPod('us-east-1', 'DevNet-Ignite', pod_number, delete=delete)
        overall_fail_count += aws_pod.run_all()
...
def get_pods():
    ...
    aws_pods_info = AwsPod.get_pods()
    return scc_pods, aws_pods_info


def cleanup_all(delete):
    scc_pods, aws_pods_info = get_pods()
    all_pods = set()
    all_pods.update(scc_pods)
    all_pods.update([i['pod_number'] for i in aws_pods_info])

    for i in range(1, ITERATION_COUNT + 1):
        pods_still_needing_cleanup = set()
        ...
        for pod_number in list(all_pods):
            ...
            current_pod_overall_fail_count = 0

            if pod_number in scc_pods:
                scc = SccPod('DevNet-Ignite', pod_number, delete=delete)
                current_pod_overall_fail_count += scc.run_all()

            matching_aws_info = [info for info in aws_pods_info if info['pod_number'] == pod_number]
            if matching_aws_info:
                aws_pod_instance = AwsPod(matching_aws_info[0]['region'], 'DevNet-Ignite', pod_number, delete=delete)
                current_pod_overall_fail_count += aws_pod_instance.run_all()

            if delete:
                if current_pod_overall_fail_count > 0:
                    print(f"Cleanup not complete for pod {pod_number}...")
                    pods_still_needing_cleanup.add(pod_number)
                else:
                    print(f"Cleanup complete for pod {pod_number}!")
            else:
                pass  # If not deleting, we just run once and break the outer loop.

        if not delete:
            break

        all_pods = pods_still_needing_cleanup

        if not all_pods:
            print("All pods cleanup complete! Exiting")
            break
