...
class NgfwInstance (CiscoEc2Instance):
    ...
    def configure_manager(self):
        ...
        Raises:
        """
        if self.fmc.is_cdfmc:
            cmd = self.reg_command
        else:
...
class ManagedDevice(NgfwInstance):
    ...
    def onboard_device_scc(self):
        """
        Purpose:    Onboards Device to SCC
        Parameters:
        Returns:    SUCCESS, FAIL
        Raises:
        """
        logger.info("Onboarding FTDv: " + self.vm_name + " to cdFMC with policy id: " + self.fmc.a_policy_name)
        r = self.fmc.onboard_device_scc(self.vm_name, self.fmc.a_policy_id, self.l_caps, self.performance_tier)
        logger.debug("Onboarding response was: " + str(r.json()))
        device_id = r.json()['entityUid']
        transaction_id = r.json()['transactionUid']
        finished = False
        while not finished:
            time.sleep(2)
            r = self.fmc.onboard_device_scc_poll(transaction_id)
            logger.debug("Polling onboarding device: " + str(r.json()))
            if r.json()['cdoTransactionStatus'] == "DONE":
                finished = True
            elif r.json()['cdoTransactionStatus'] == "ERROR":
                raise Exception("Onboarding device failed: " + str(r.json))
        r = self.fmc.get_device_scc(device_id)
        self.reg_command = r.json()['cdFmcInfo']['cliKey']
    ...
    def send_registration_request(self):
        ...
        if reg_task_id is not None:
            ...
            if self.reg_sts != "COMPLETED":
                return 'FAIL'
        if self.fmc.is_cdfmc:
            logger.info("Adding device to group in cdFMC")
            r = self.fmc.add_member_in_device_grp(self.vm_name, self.fmc.d_grp_name, self.fmc.d_grp_id)
    ...
    def remove_from_fmc(self):
        ...
        try:
            if self.fmc.is_cdfmc:
                r = self.fmc.deregister_device_scc(self.vm_name)
            else:
