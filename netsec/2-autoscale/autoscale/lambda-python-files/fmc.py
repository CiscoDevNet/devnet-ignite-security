...
class FirepowerManagementCenter:
    ...
    """
    def __init__(self, fmc_server, username=None, password=None, api_token=None, accesspolicy=None):
        if (username is None or password is None) and api_token is None:
            raise Exception("Init Error --> provide username and password for FMC or api_token for SCC")
        self.server = 'https://' + fmc_server
        self.is_cdfmc = True if api_token else False
        if self.is_cdfmc:
            self.api_token = api_token
            self.scc_server = self.server
            self.server = self.server + '/firewall/v1/cdfmc'
    ...
    def add_member_in_device_grp(self, device_name, grp_name, grp_id):
        """
        Purpose:    Add device to grp id
        Parameters: Device ID, Group Id
        Returns:    response
        Raises:
        """
        api_path = "/api/fmc_config/v1/domain/e276abec-e0f2-11e3-8169-6d9ed49b625f/devicegroups/devicegrouprecords/"
    ...
    def onboard_device_scc(self, name, policy_id, license_caps, performance_tier):
        """
        Purpose:    Onboard the device to SCC
        Parameters: Name of device, Access Policy Id, Licenses Cap
        Returns:    REST post response
        Raises:
        """
        logger.info("Onboarding: "+name)
        api_path = "/firewall/v1/inventory/devices/ftds"
        url = self.scc_server + api_path
        post_data = {
            "name": name,
            "deviceType": "CDFMC_MANAGED_FTD",
            "virtual": True,
            "licenses": license_caps,
            "performanceTier": performance_tier,
            "fmcAccessPolicyUid": policy_id
        }

        r = self.rest_post(url, post_data)
        return r

    def onboard_device_scc_poll(self, transaction_id):
        """
        Purpose:    Poll onboarding the device to SCC
        Parameters: Transaction ID
        Returns:    REST get response
        Raises:
        """
        api_path = f"/firewall/v1/transactions/{transaction_id}"
        url = self.scc_server + api_path
        r = self.rest_get(url)
        return r

    def register_device_scc(self, name):
        """
        Purpose:    Onboard the device to SCC
        Parameters: Name of device, Access Policy Id, Licenses Cap
        Returns:    REST post response
        Raises:
        """
        api_path = "/firewall/v1/inventory/devices/ftds/register"
        dev_id = self.get_device_id_by_name_scc(name)
        logger.info("Registering to cdFMC: "+ name + " uid: " + dev_id)
        url = self.scc_server + api_path
        post_data = {
            "ftdUid": dev_id,
            "skipInitialDeployment": False
        }

        r = self.rest_post(url, post_data)
        return r
        
    def get_device_scc(self, device_id):
        """
        Purpose:    Get the device info from SCC
        Parameters: Device ID
        Returns:    REST get response
        Raises:
        """
        api_path = f"/firewall/v1/inventory/devices/{device_id}"
        url = self.scc_server + api_path
        r = self.rest_get(url)
        return r

    def deregister_device_scc(self, name):
        """
        Purpose:    De-registers the device from SCC
        Parameters: Device Name
    ...
    def get_device_id_by_name_scc(self, name):
        ...
        """
        api_path = "/firewall/v1/inventory/devices"
        url = self.scc_server + api_path + '?offset=0&limit=200'
        r = self.rest_get(url)
        if 'items' in r.json():
            for item in r.json()['items']:
                if item['name'] == name:
    ...
    def start_deployment(self, device_name):
        ...
        if device_name in device_list:
            ...
            if 'error' in r.json():
                if 'messages' in r.json()['error']:
                    if 'in progress' in r.json()['error']['messages']:
                        return "in progress"
        return None
...
class DerivedFMC(FirepowerManagementCenter):
    ...
    """
    def __init__(self, fmc_server, username=None, password=None, api_token=None, accesspolicy=None):
        super().__init__(fmc_server, username, password, api_token, accesspolicy)
    ...
    def reach_fmc_(self):
        ...
        try:
            self.get_auth_token()
            if self.headers.get('X-auth-access-token') or self.is_cdfmc:
        ...
        if self.reachable == 'AVAILABLE':
            ...
            if self.a_policy_id == '':
                ...
                if not utl.find_value_in_list(r, self.d_grp_id):
                    names, ids = self.get_member_list_in_device_grp(self.d_grp_id)
                    present = False
                    for device_id in ids:
                        if utl.find_value_in_list(r, device_id):
                            present = True
                    if not present:
                        return self.configuration_status
    ...
    def register_ftdv(self, vm_name, mgmt_ip, reg_id, nat_id, license_caps, performance_tier):
        ...
        if self.is_cdfmc:
            try:
                logger.info("Registering FTDv: " + vm_name + " to cdFMC")
                r = self.register_device_scc(vm_name)
                logger.debug("Register response was: " + str(r.json()))
                if 'cdoTransactionStatus' in r.json():
                    return r.json()['cdoTransactionStatus']
            except Exception as e:
                logger.exception(e)
                return None
        else:
