...
class EC2Instance:
    ...
    def __init__(self, instance_id, group=None):
        ...
        self.vm_name = self.get_instance_name()
        if not self.vm_name:
    ...
    def get_instance_name(self):
        """
        Purpose:        To get instance Tag Name
        Parameters:
        Returns:        Instance tag Name or None
        Raises:
        """
        name = None
        response = self.__get_describe_instance()
        if response is not None:
            ...
            logger.info("No Name tag provided for instance")
            return name
    ...
    def get_private_ip_of_interface(self, interface_suffix):
        ...
        eni_name = name + interface_suffix
        try:
    ...
    def get_subnet_id_of_interface(self, interface_suffix):
        ...
        """
        name = self.get_instance_name()
        if not name or 'NGFWv-AutoScale' in name:
            name = self.instance_id
        eni_name = name + interface_suffix
        try:
            result = self.ec2.describe_network_interfaces(Filters=[{'Name': 'tag:Name', 'Values': [eni_name]}])
